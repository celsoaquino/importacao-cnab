package com.celsoaquino.backend.job;

import com.celsoaquino.backend.entity.Transaction;
import com.celsoaquino.backend.entity.TransactionCNAB;
import com.celsoaquino.backend.entity.TypeTransaction;
...
public class BatchConfig {
    ...
    ItemProcessor<TransactionCNAB, Transaction> processor() {
        return item -> {
             var typeTransaction = TypeTransaction.findByType(item.tipo());
             var normalizedValue = item.valor()
                 .divide(new BigDecimal(100))
                 .multiply(typeTransaction.getSignal());

            var trasaction = new Transaction(
                    null, item.tipo(), null,
                    normalizedValue, item.cpf(),
    ...
    @Bean
    JobLauncher jobLauncherAsync(JobRepository jobRepository) {
